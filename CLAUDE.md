...
$PY -m pytest -q -p no:warnings              # 51 tests, ~5s; includes a real Shiny session test
...
`run_loop()` writes actions and `state=running`, then loops: poll commands (every `poll_every` steps, continuously while paused) -> if paused sleep 0.1s -> `maybe_snapshot` -> eval -> eval artifacts -> `train_step_fn()` -> `step += 1` -> throttled status write. The snapshot must stay ahead of eval: eval may consume RNG, and a snapshot taken after it made every rewind onto an eval step (all disk steps are) replay different batches than the parent branch. Exceptions write `state=crashed` with the error string and re-raise; `KeyboardInterrupt` writes `interrupted`; `run.finalize()` always runs. Commands are `{"type": name, **args}`; args are coerced against the registered `ActionSpec` (`registry.coerce_args`) before the handler runs, and a failing or unknown command is recorded as a `failed` event while the loop continues. `register_handler(name, fn, spec=ActionSpec(...))` adds a project command; `spec.name` must equal `name`. Handlers that mutate weights should call `controller.snapshot_now()` first.

### Snapshots and branching

`SnapshotManager` (`snapshot.py`) keeps `TrainerState` objects (CPU model/optimizer state plus torch/python/numpy/cuda RNG) keyed by `(branch_id, step)`: an in-memory ring (`ring_every=10`, `ring_size=20`) and disk via `track_artifact(group="trainer_state")` every `disk_every=200`. Disk snapshots are written as `TrainerState.to_dict()` and read back with `TrainerState.from_dict()`: a plain dict of tensors, lists and scalars (numpy's RNG keys become an int64 tensor, python's state tuple becomes lists). Never hand the dataclass itself to `track_artifact(type="torch")`: TrackLab loads with a default-argument `torch.load`, which is `weights_only=True` since PyTorch 2.6 and rejects pickled classes; that is what made every rewind to a ring-evicted step fail. `tests/test_snapshot.py::TorchDiskRun` guards this with a real `torch.save`/`torch.load` roundtrip. First write wins per `(branch_id, step)`: a handler's `snapshot_now()` runs before the scheduled `maybe_snapshot` in the same loop iteration, so the pre-intervention state is the one kept in the ring and written to disk; `maybe_snapshot` reuses an existing ring entry instead of re-capturing. This matters because a fork step is always a multiple of `disk_every`, so "rewind then perturb" in one poll used to overwrite the pre-perturb snapshot with the perturbed one. "Rewind to S" means the state before anything that happened at step S. Cadence is not yet configurable from `TrainerController` (Milestone 2). `rewind` restores `nearest_before(step, branch)`, walking parent lineage capped at fork steps, creates branch `b<n>@t<step>`, and writes a `fork` event. Only model, optimizer and RNG are restored: LR schedulers, data iterators and scalers drift after a rewind unless batches derive from the torch RNG (Milestone 2 adds state hooks).
...
`metrics_panel.py` builds its Plotly figure once per *structure* (ordered metrics plus the set of `(metric, branch_id)` traces, held in a `reactive.value` that is only `set()` on change) and pushes new x/y arrays into the existing traces of `chart.widget` inside `batch_update()`. Do not go back to returning a fresh `px.line` figure from the renderer on every poll: shinywidgets turns each returned figure into a new `FigureWidget`, whose comm-open message carries the multi-megabyte plotly.js bundle, and the browser rebuilds the whole plot every second (visible lag, flicker, high CPU). The pure helpers (`parse_rows`, `structure_key`, `build_figure`, `push_points`) are unit-tested without a session in `tests/test_metrics_panel.py`; the e2e test only checks the widget renders.
